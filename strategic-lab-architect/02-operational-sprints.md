...
2. **Substitution Logic:** Define the Value Proposition not as a 'new feature,' but as the replacement of a specific, high-friction expense or **P&L Invoice** the customer is already paying. Identify that invoice. Is it a consulting fee, an internal payroll cost, or a legacy software subscription? Explain why this specific budget line is vulnerable. If the archetype involves data review or auditing, you **MUST** use the **'Census vs. Sampling'** logic: quantify the 'Sampling Gap' (what humans miss) as the primary source of 'Found Money'."
...
### Task 3.1: MVP Critical Path & Technical Stack
"Initialize Sprint 3.1. Define the **MVP Critical Path**. 
1. **The Stack:** Provide the 'Hard Specs'—Software stack, API requirements, hardware/IoT needs, and specific cloud compliance levels (e.g., SOC2, FedRAMP, HIPAA). 
2. **The Tooling:** Identify the essential third-party tools (SaaS/Infrastructure) required for the core engine. 
3. **The 'Build vs. Buy' Logic:** Justify which components must be proprietary to protect the USP and which should be outsourced to maintain an **Asset-Light** posture."
...
### Task 4.3: The Valley of Death (18-Month Cashflow)
"Initialize Sprint 4.3. Model the cashflow for the first 18 months of operation.
1. **The Fortune 500 Float:** Explicitly account for the **Payment Latency** (Net-90 or Net-120 terms).
2. **The Cumulative Cash Bottom:** Map the 'Deepest Point' in the cash curve. When does the bank account reach its lowest point before turning positive?
3. **The Structural Bridges:** Propose 2 ways to survive this gap (e.g., Implementation fees paid on signature, Venture Debt/Factoring, or Tiered milestone payments)."
